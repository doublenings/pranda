		...
		// Section
			...
			$this->add_control(
				...
				[
					...
					'type' => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .naeep-upcoming-item' => 'background-color: {{VALUE}};',
					],
				]
			);
			$this->add_group_control(
			...
			<div class="naeep-upcoming">
				...
				foreach ( $eventItem as $each_logo ) {
			  	...
			  	$title_link = !empty( $each_logo['title_link']['url'] ) ? $each_logo['title_link']['url'] : '';
					...
					$link_title = $title_link ? '<a href="'.esc_url($title_link).'" '.$title_link_attr.'>'.esc_html($upcoming_title).'</a>' : esc_html($upcoming_title);
					$title = $upcoming_title ? '<h3 class="upcoming-title">'.$link_title.'</h3>' : '';
			  	$time = !empty( $upcoming_time ) ? '<span class="time">'.esc_html($upcoming_time).'</span>' : '';
					$speaker = $upcoming_speaker ? '<h5>'.esc_html($upcoming_speaker).'</h5>' : '';
					$venue = $upcoming_venue ? '<span class="venue">'.esc_html($upcoming_venue).'</span>' : '';
					?>
					<div class="naeep-upcoming-item">
