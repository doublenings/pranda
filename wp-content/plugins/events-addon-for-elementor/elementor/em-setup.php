	  ...
	  */
		...
		*/
		public function __construct(){
			...
			add_action( 'elementor/editor/before_enqueue_scripts', function() {
			   wp_enqueue_style('naevents-ele-editor-linea', NAEEP_PLUGIN_URL . 'assets/css/linea.min.css', [], '1.0.0');
			   wp_enqueue_style('naevents-ele-editor-themify', NAEEP_PLUGIN_URL . 'assets/css/themify-icons.min.css', [], '1.0.0');
			} );

			// Js Enqueue
			add_action( 'elementor/frontend/after_enqueue_scripts', function() {

   			wp_enqueue_script( 'naevents-chartjs', NAEEP_PLUGIN_URL . 'assets/js/Chart.min.js', array( 'jquery' ), '2.2.1', true );
				wp_enqueue_script( 'naevents-elementor', NAEEP_PLUGIN_URL . 'elementor/js/naevents-elementor.js', [ 'jquery' ], false, true );
			} );
		...
		*/
		public static function controls_helper(){
			...
			foreach($group_controls as $control){
				if ( file_exists( NAEEP_EM_SHORTCODE_BASE_PATH . '/lib/'.$control.'.php' ) ){
					require_once( NAEEP_EM_SHORTCODE_BASE_PATH . '/lib/'.$control.'.php' );
				}
			}
		}
		...
		*/
		public function naevents_basic_widgets_registered(){
			// init widgets
			$basic_dir = NAEEP_EM_BASIC_SHORTCODE_PATH;
			// Open a directory, and read its contents
			if (is_dir($basic_dir)){
			  $basic_dh = opendir($basic_dir);
		    while (($basic_file = readdir($basic_dh)) !== false){
		    	if (!in_array(trim($basic_file), ['.', '..'])) {
						$basic_template_file = NAEEP_EM_SHORTCODE_BASE_PATH . 'widgets/basic/'.$basic_file;
		...
		public function naevents_widgets_registered(){
			// init widgets
			$dir = NAEEP_EM_SHORTCODE_PATH;
			// Open a directory, and read its contents
			if (is_dir($dir)){
			  $dh = opendir($dir);
		    while (($file = readdir($dh)) !== false){
		    	if (!in_array(trim($file), ['.', '..'])) {
						$template_file = NAEEP_EM_SHORTCODE_BASE_PATH . 'widgets/event/'.$file;
		...
		public function naevents_unique_widgets_registered(){
			// init widgets
			$unique_dir = NAEEP_EM_UNIQUE_SHORTCODE_PATH;
			// Open a directory, and read its contents
			if (is_dir($unique_dir)){
			  $unique_dh = opendir($unique_dir);
		    while (($unique_file = readdir($unique_dh)) !== false){
		    	if (!in_array(trim($unique_file), ['.', '..'])) {
						$unique_template_file = NAEEP_EM_SHORTCODE_BASE_PATH . 'widgets/event-unique/'.$unique_file;
