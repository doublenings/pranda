<?php
if(!class_exists('Aglee_Lite_Welcome')) :

	class Aglee_Lite_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					...
					'more_wp' => esc_html__('More WordPress Stuff', 'aglee-lite'),
				);
			...
			public function welcome_admin_notice_display(){
				...
				$aglee_lite_an_div = '
				<div class="updated aglee-lite-an notice notice-success is-dismissible">
				%1$s
				<hr/>
				<div class="aglee-lite-column-wrap">
				<div class="aglee-lite-column col-img">%2$s</div>
				<div class="aglee-lite-column col-demos">%3$s</div>
				<div class="aglee-lite-column col-doc">%4$s</div>
				</div> 
				</div>
				<style>%5$s</style>';

				/* translators: 1 - notice title, 2 - notice message */
				$aglee_lite_an_top = sprintf(
					'<h2>%1$s</h2><p class="about-desc">%2$s</p></hr>',
					esc_html__( 'Congratulations!', 'aglee-lite' ),
					sprintf(
						/* translators: %s - theme name */
						esc_html__( '%s is now installed and ready to use. We\'ve assembled some links to get you started.', 'aglee-lite' ),
						$this->theme_name
					)
				);

				$aglee_lite_an_btn = sprintf(
					/* translators: 1 - url, 2 - button text */
					'<a href="%1$s" class="button button-primary aglee-lite-btn" >%2$s</a>',
					esc_url( admin_url( 'themes.php?page=welcome-page#demo_import' ) ),
					esc_html__( 'Try one of our ready to use Starter Sites', 'aglee-lite' )
				);
				$aglee_lite_an_pbtn = sprintf(
					...
					esc_html__( 'or go to the theme settings', 'aglee-lite' )
				);
				$aglee_lite_an_img = sprintf(
					'<picture>
				...
				$aglee_lite_an_list = sprintf(
					'<div><h3><span class="dashicons dashicons-images-alt2"></span> %1$s</h3><p>%2$s</p></div><div> <p>%3$s</p><p>%4$s</p> </div>',
					__( 'Sites Library', 'aglee-lite' ),
					// translators: %s - theme name
					sprintf( esc_html__( '%s now comes with a sites library with various designs to pick from. Visit our collection of demos that are constantly being added.', 'aglee-lite' ), $this->theme_name ),
					$aglee_lite_an_btn,
					$aglee_lite_an_pbtn
				);
				$aglee_lite_an_doc = sprintf(
					'<div><h3><span class="dashicons dashicons-format-aside"></span> %1$s</h3><p>%2$s</p><a class="button aglee-lite-btn" href="%3$s">%4$s</a></div><div></div>',
					__( 'Documentation', 'aglee-lite' ),
					// translators: %s - theme name
					sprintf( esc_html__( 'Need more details? Please check our full documentation for detailed information on how to use %s.', 'aglee-lite' ), $this->theme_name ),
					'https://8degreethemes.com/documentation/aglee-lite',
					esc_html__( 'Read full documentation', 'aglee-lite' )
				);
				$aglee_lite_an_style = '
				.aglee-lite-an.notice.is-dismissible {padding: 20px 15px 5px;}
				.aglee-lite-an hr {margin: 20px -23px 0;border-top: 1px solid #f3f4f5;border-bottom: none;}
				.aglee-lite-an h3{margin: 17px 0 0;font-size: 16px;line-height: 1.4;}
				.aglee-lite-an p {color: #72777c;}
				.aglee-lite-an h2{margin: 0;font-size: 21px;font-weight: 400;line-height: 1.2;}
				.aglee-lite-an p.about-desc{color: #72777c;font-size: 16px;margin: 0;padding:0px;}
				.aglee-lite-column-wrap {display: -ms-grid;display: grid;-ms-grid-columns: 24% 32% 32%;grid-template-columns: 24% 32% 32%;margin-bottom: 13px;}
				.aglee-lite-column {padding: 20px 40px 0 0;}
				.aglee-lite-an .aglee-lite-btn {padding: 30px;line-height: 0;}';
				...
				<div class="wrap about-wrap access-wrap">					
					<div class="top-block-wrap">					
						<div class="about-text">
							<h1><?php /* translators: %1$s: theme name, %2$s: theme version */ printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'aglee-lite' ), esc_html($this->theme_name), esc_html($this->theme_version) ); ?></h1>
						...
						<div class="welcome-section-wrapper is_loading">
							...
							<div class="notice-sidebar is_loading">
								<div class= "notice-sidebar-item">
									<h4><?php echo esc_html('Join in our social networks!','aglee-lite') ?></h4>
									<p><?php echo esc_html__('Get connected, share your opinions and more via our social community:', 'aglee-lite'); ?></p>
									<p><a href="<?php echo esc_url('https://www.facebook.com/8DegreeThemes');?>"><?php echo esc_html('Join our Facebook Group','aglee-lite') ?></a> <?php echo esc_html('- to receive updates, offers and more.','aglee-lite') ?></p>
									<p><a href="<?php echo esc_url('https://www.youtube.com/8degreethemes');?>"><?php echo esc_html('Subscribe our YouTube Channel','aglee-lite') ?></a> <?php echo esc_html('- for tutorials, videos and more.','aglee-lite') ?></p>
									<p><a href="<?php echo esc_url('https://www.twitter.com/8degreethemes');?>"><?php echo esc_html('Follow us on Twitter','aglee-lite') ?></a> <?php echo esc_html('- to stay updated.','aglee-lite') ?></p>
								</div>
								<div class= "notice-sidebar-item">
									<h4><?php echo esc_html('Leave us a review','aglee-lite') ?></h4>
									<p><?php echo esc_html__('Are you are enjoying aglee Lite? We would love to hear your feedback.', 'aglee-lite'); ?></p>
									<a href="<?php echo esc_url('https://wordpress.org/support/theme/aglee-lite/');?>"><?php echo esc_html('Submit a review','aglee-lite') ?></a>
			...
			public function plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') ) {
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'aglee-lite' ) );
				}
				...
				if (! wp_verify_nonce( $nonce, 'plugin_installer_nonce' )) {
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'aglee-lite') );
				}
			...
			public function plugin_activation_callback() {
				...
				if(file_exists($plugin_file)) {
					...
					echo esc_html__('Plugin Does not Exists' , 'aglee-lite');
				}
